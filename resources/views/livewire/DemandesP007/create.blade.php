...
$(".next").click(function () {
    if (!testSize()) {
        ...
        //is_producteur is_importateur is_distributeur
        current_fs.find('input[name=is_producteur]').each(function () {
        //if ($(this).val() === "") {
        if (!$("input[name=is_producteur]").is(":checked") && !$("input[name=is_importateur]").is(":checked") && !$("input[name=is_distributeur]").is(":checked")) {
                isValid = false;
                $("#errorCheckbox").show();
            } else {
                $("#errorCheckbox").hide();
